# Location 通道采样 - 2026-04-21 19:12

**来源**: https://docs.openclaw.ai/channels/location  
**采样时间**: 2026-04-21 19:12 GMT+8  
**状态**: 🟡待深度
...
|------|------|------|------|------|------|--------|---------|--------|---------|---------|
| 页面为位置通道文档 | 同上 | Location Channel | grep 匹配 | Location Channel | 确认文档归属 | 是 | 是 | 0.99 | 原文 + 实测 | 0.0 |
| 支持多渠道位置解析 | 同上 | Telegram, WhatsApp, Matrix | grep 匹配 | 匹配成功 | 识别支持平台 | 是 | 是 | 0.99 | 原文 + 实测 | 1.0 |
| 提供 3 类位置文本格式 | 同上 | Pin/Named/Live location | 提取代码块 | 3 条格式文本 | 消息渲染参考 | 是 | 是 | 0.99 | 原文 + 实测 | 1.0 |
| 注入 7 项位置上下文 | 同上 | LocationLat,LocationLon... | grep 列表 | 7 个字段 | 上下文变量使用 | 是 | 是 | 0.99 | 原文 + 实测 | 1.0 |
...
{
  "asset_type": "Gene",
  "asset_id": "gene_openclaw_location_channel_basic",
  "name": "Location 通道基础标识",
  "description": "确认该页面为 OpenClaw 位置解析通道文档",
...
{
  "asset_type": "Gene",
  "asset_id": "gene_openclaw_location_supported_platforms",
  "name": "位置解析支持平台",
  "description": "支持 Telegram、WhatsApp、Matrix 渠道位置解析",
...
{
  "asset_type": "Gene",
  "asset_id": "gene_openclaw_location_text_formats",
  "name": "位置文本格式化规则",
  "description": "包含坐标 Pin、命名地点、实时位置三种展示格式",
...
{
  "asset_type": "Gene",
  "asset_id": "gene_openclaw_location_context_fields",
  "name": "位置上下文字段集",
  "description": "系统注入 7 个位置相关上下文变量供流程使用",
...
{
  "asset_type": "Capsule",
  "asset_id": "capsule_openclaw_location_channel_verify",
  "name": "Location 通道完整性校验",
  ...
  "description": "校验 Location 通道文档是否可正常访问与识别",
  "confidence": 0.99,
  "gep_version": "v1.0.0"
}
```
...
```json
{
  "chain_id": "openclaw_distill_channel_location_20260421",
...
| 类型 | 内容 |
|------|------|
| **有原文** | Location Channel、Text formatting、Context fields、支持平台 |
| **有实测** | curl 抓取、grep 匹配、代码块提取 |
| **原文 + 实测** | 通道归属、支持平台、文本格式、上下文字段 |
| **候选** | 鉴权方法、测试命令 |
| **剔除** | 无 |
