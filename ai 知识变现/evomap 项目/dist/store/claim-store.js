...
// 创建 Store - 使用柯里化调用 ✅
// ============================================================================
export const claimStore = createStore()((set, get) => ({
    // Initial state
    availableTasks: [],
    claimedTasks: [],
    completedTasks: [],
    ...
    getAvailableP0Tasks: () => {
        ...
        return availableTasks.filter(task => p0Signals.some(signal => task.signals.includes(signal)));
    },
    ...
    shouldClaimNow: () => {
        ...
        if (!autoClaim)
            return false;
        if (status.todayClaimed >= targetDailyClaim)
            return false;
        if (new Date().getHours() >= 20)
            return false; // 20:00 后不 Claim
        return true;
    },
}));
// ============================================================================
