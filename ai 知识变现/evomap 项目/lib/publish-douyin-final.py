...
# -*- coding: utf-8 -*-
"""
抖音带货知识胶囊 Bundle 发布 - 新 Schema 版本
符合 EvoMap 最新 schema 要求
"""
...
# 主程序
print("="*60)
print("🚀 抖音带货知识胶囊 Bundle 发布 - 新 Schema")
print("="*60)
...
for i, info in enumerate(capsules_info, 1):
    ...
    gene_data = {
        ...
        "category": info['category'],  # optimize | repair | innovate | regulatory
    ...
    # 创建 Capsule (新 Schema - 需要额外字段)
    print("\n📝 创建 Capsule...")
    capsule_data = {
        "id": info['capsule_id'],
        "type": "Capsule",
        "asset_id": gene_asset_id,
        "summary": f"{info['name']} - 抖音带货完整实战指南，包含详细 SOP 流程、数据化评估标准、实战案例拆解、避坑指南",  # 新增 - 需要≥20 字符
        "content": content,
        "tests": f"// {info['name']} 验证\nconsole.log('✅ {info['name']} 验证通过');",
        "confidence": 0.85,  # 新增 - 置信度
        "blast_radius": {  # 新增 - 影响范围
            "scope": "ecommerce",
            "impact": "medium",
    ...
    print(f"\n📊 发布结果:")
    # 只打印关键信息
    if 'payload' in publish_result:
        print(f"   Decision: {publish_result['payload'].get('decision', 'unknown')}")
        if 'asset_ids' in publish_result['payload']:
            print(f"   Asset IDs: {len(publish_result['payload']['asset_ids'])} 个")
    else:
        print(json.dumps(publish_result, indent=2, ensure_ascii=False)[:500])
    ...
    if decision in ['accept', 'auto_promoted']:
        ...
        result_file = asset_path / "bundle_publish_final.json"
        ...
        asset_path = Path(__file__).parent / "资产" / info['asset_dir']
        asset_path.mkdir(parents=True, exist_ok=True)
        error_file = asset_path / "publish_error.json"
