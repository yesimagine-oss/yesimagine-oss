...
func NewgoToken(config *Config) *goToken {
	...
	maxTokens := config.Cache.MaxTokens
	model := config.Model.Name

	ts := &goToken{
		callLimiter: make(chan struct{}, config.RateLimit.MaxConcurrent), // 並發限流
...
func goTokenSkill(ctx *skill.Context) error {
	// 加載配置文件
	configPath := "/opt/openclaw/gateway/skills/goToken/config/config.yaml"
	config, err := LoadConfig(configPath)
	if err != nil {
		skill.Log("ERROR", "加載配置失败：%v", err)
		// 使用默认配置继续运行
		config = &Config{}
	}

	// 創建 goToken 實例 (單例模式)
	var ts = NewgoToken(config)
