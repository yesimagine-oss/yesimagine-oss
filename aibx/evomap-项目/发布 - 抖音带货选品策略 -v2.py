...
def publish():
    ...
    capsule = {
        ...
        "tests": [
            "Test product selection formula calculates correctly",
    ...
    if hello_result.get("success") or hello_result.get("status") == "ok":
        hub_node_id = hello_result.get("hub_node_id") or hello_result.get("data", {}).get("hub_node_id")
        print(f"✅ Hello 成功：Hub Node ID = {hub_node_id}")
    else:
        print(f"⚠️ Hello 响应：{json.dumps(hello_result, ensure_ascii=False)[:200]}")
        hub_node_id = NODE_ID
    ...
    gene_envelope = {
        ...
        "message_id": f"msg_{int(datetime.now().timestamp()*1000)}_gene",
        "sender_id": NODE_ID,
    ...
    publish_resp = requests.post(publish_url, json=gene_envelope, headers=headers, timeout=30)
    publish_result = publish_resp.json()
    
    print(f"Gene 发布状态码：{publish_resp.status_code}")
    print(f"Gene 发布结果：{json.dumps(publish_result, ensure_ascii=False)[:500]}")
    ...
    capsule_envelope = {
        ...
        "protocol_version": "1.0.0",
        "message_type": "publish",
        "message_id": f"msg_{int(datetime.now().timestamp()*1000)}_capsule",
        "sender_id": NODE_ID,
        "timestamp": datetime.utcnow().isoformat() + "Z",
        "payload": {
            "action": "publish",
    ...
    publish_resp = requests.post(publish_url, json=capsule_envelope, headers=headers, timeout=30)
    publish_result = publish_resp.json()
    
    print(f"Capsule 发布状态码：{publish_resp.status_code}")
    print(f"Capsule 发布结果：{json.dumps(publish_result, ensure_ascii=False)[:500]}")
