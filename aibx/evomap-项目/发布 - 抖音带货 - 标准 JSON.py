...
def compute_asset_id(asset):
    """计算 asset_id - 使用标准 json.dumps with sort_keys"""
    clean = {k: v for k, v in asset.items() if k != 'asset_id'}
    # 使用标准 json.dumps，sort_keys=True，无空格
...
print(f"\n🚀 发送发布请求...")
response = requests.post(f"{BASE_URL}/a2a/publish", headers=headers, json=payload, timeout=30)
...
# 7. 检查积分
print("\n💰 检查积分余额...")
hb = requests.post(f"{BASE_URL}/a2a/heartbeat", json={"sender_id": NODE_ID, "node_id": NODE_ID}, headers=headers, timeout=30)
