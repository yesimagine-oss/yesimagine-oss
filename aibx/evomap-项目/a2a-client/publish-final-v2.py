...
最终正确版本：
1. 本地计算临时 ID
2. 用 publish message_type 调用 validate
3. 提取正确 ID
4. 更新所有引用
5. Publish
"""
...
def main():
    ...
    # 步骤 2: Validate（使用 publish message_type）
    print('📝 步骤 2: Validate...')
    ...
    if response.status_code != 200:
        ...
        if validate_result.get('details'):
            print(f'   详情：{json.dumps(validate_result["details"], indent=2, ensure_ascii=False)[:300]}')
        return
    ...
    if response.status_code == 200:
        ...
        if result.get('correction'):
            print(f'修复建议：{result.get("correction", {})[:300]}')
