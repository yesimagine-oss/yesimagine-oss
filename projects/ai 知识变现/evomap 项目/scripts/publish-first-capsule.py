...
NODE_SECRET = "1a747f906e529fb1dde405b04b0f8d3ab20724737810a22e75e7af4bf741aa6d"
...
def publish_capsule():
    ...
    # 先執行 Hello 認證
    print("\n🔑 執行 Hello 認證...")
    hello_client = requests.Session()
    hello_payload = {
        "protocol": "gep-a2a",
        "protocol_version": "1.0.0",
        "message_type": "hello",
        "message_id": f"msg_{int(datetime.now().timestamp())}_hello",
        "sender_id": NODE_ID,
    ...
    try:
        ...
        if hello_resp.status_code == 200:
            ...
            # 獲取最新的 Node Secret（如果有）
            new_secret = hello_data.get("payload", {}).get("node_secret")
            if new_secret:
                print(f"🔄 更新 Node Secret")
                # 使用新的 Node Secret
    except Exception as e:
        print(f"⚠️ Hello 認證異常：{e}")
  ...
  async def reconnect(self, ws_factory):
    ...
    # 添加 asset_id 到每個資產
    payload["payload"]["assets"][0]["asset_id"] = gene_id
    payload["payload"]["assets"][1]["asset_id"] = capsule_id
    payload["payload"]["assets"][2]["asset_id"] = event_id
    
    # 6. 發送請求
    print(f"\n🚀 發送發布請求...")
    print(f"  Node ID: {NODE_ID}")
    print(f"  資產數量：3 (Gene + Capsule + Event)")
    print(f"  描述：{payload['payload']['description']}")
