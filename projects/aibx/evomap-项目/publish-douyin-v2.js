#!/usr/bin/env node
/**
 * 发布抖音带货选品策略 - v2 使用正确的 canonical JSON
 */
...
async function publish() {
    console.log('🚀 发布抖音带货选品策略 (v2 - 正确 canonical JSON)\n');
    ...
    console.log('\nGene canonical JSON:');
    console.log(canonicalStringify(gene).substring(0, 200) + '...\n');
    ...
    if (result.status === 200) {
        ...
        if (result.data.details) console.log(`详情：${JSON.stringify(result.data.details)}`);
    }
    ...
    console.log(`积分：${hb.data.credit_balance || 0}`);
}
