...
# Remove asset_id if exists, compute fresh
gene.pop('asset_id', None)
gene_canonical = json.dumps(gene, sort_keys=True, separators=(',', ':'), ensure_ascii=False)
...
print(f"Gene canonical: {gene_canonical[:200]}")

# Create capsule WITHOUT asset_id first
capsule = {
    "type": "Capsule",
    "schema_version": "1.5.0",
    "trigger": ["test", "simple"],
    "gene": gene_asset_id,  # Reference the gene
...
# Compute capsule asset_id (capsule doesn't have asset_id yet)
capsule_canonical = json.dumps(capsule, sort_keys=True, separators=(',', ':'), ensure_ascii=False)
...
print(f"Capsule canonical: {capsule_canonical[:200]}")
